...
const { projectName } = require('../index');
...
function extractHubbleContent(filePath) {
    try {
        ...
        const contentWithoutScript = hubbleContent.replaceAll('`', '\\`')
        console.log("avant",contentWithoutScript,"with script");
...
function FolderHubbleTraitement() {
    const hubblePath = __dirname + `/../../../client/src/pages`;
    const widgetsPath = __dirname + `/../../../client/src/widgets`;
    ...
    try {
        ...
        return allContent+ routeComponnent +addRuntimeCode();
